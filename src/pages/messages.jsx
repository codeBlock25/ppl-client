...
class Messages extends Component {
  ...
  async getMsgs() {
    await Axios({
      method: "GET",
      url: `${apiUrl}/message?from=${"amosdaniel252@gmail.com"}`,
    }).then((result) => {
        ...
        .then((result) => {
          this.setState({
            msgs: [
              {
                from:
                  "amosdaniel252@gmail.com" || this.props.staffDetails.email,
                to: this.state.messagee,
                message: this.state.message,
                date: Date(),
              },
    ...
    return (
      <div className="Messages">
        ...
        {this.state.msgs.length >= 1 ? (
          this.state.msgs.map((msg) => {
            return (
              <div
                className={
                  msg.from === "amosdaniel252@gmail.com"
                    ? "message own"
                    : "message"
                }
              >
                <span>
                  {msg.from === "amosdaniel252@gmail.com" ? "To" : "From"}:{" "}
                  {msg.from === "amosdaniel252@gmail.com" ? msg.to : msg.from}
                </span>
                {msg.message}
        ...
        <form action="" onSubmit={this.sendMsg}>
          ...
          />
          <Fab type="submit" className="messageBtn">
            <Send />
          </Fab>
        </form>
      </div>
