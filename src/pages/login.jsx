...
class Login extends Component {
  ...
  async handleSubmit(e) {
    ...
    if (this.state.email === "") {
      ...
      document.querySelector(".account form .error").innerHTML =
      ...
      document.querySelector(".account form .error").classList.add("active");
      document.querySelector(".account form .error").innerHTML =
        "please you need to agree on this";
    } else {
      document.querySelector(".account form .error").classList.remove("active");
      console.log("passed");
      await Axios({
        url: `${apiUrl}/login?email=${this.state.emaild}&password=${this.state.password}`,
        method: "GET",
      })
        .then((data) => {
        ...
        <div className="wrapper">
          ...
          <form action="" onSubmit={this.handleSubmit}>
            ...
            <Button type="submit" className={classes.btn}>
              {this.state.loading ? (
                <ClipLoader color="white" size="25" sizeUnit="px" />
              ) : (
                <span>
                  login
                  <ArrowForward style={{ transform: "translate(30%, 23%)" }} />
                </span>
              )}{" "}
            </Button>
          </form>
